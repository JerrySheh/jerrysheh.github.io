...
为了改进查找的时间复杂度，于是出现了 **平衡二叉树(AVL)**，平衡二叉树使得每个结点的左结点和右结点的深度差不超过1。
...
和二叉搜索树每个结点只能存储一个元素，每个结点的孩子数不多于两个的性质不一样，多路查找树每一个结点的孩子数可以多于两个，每一个结点处都可以存储多个元素 。

![](../../../../images/DataStructure/BTree.png)

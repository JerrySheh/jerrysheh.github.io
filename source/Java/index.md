...
为什么扩容是2倍，而不是1.5倍或3倍？因为要保证table的长度为 2^n， 为什么 table 的长度要为 2^n ？ 因为这样可以均匀分布减少碰撞。计算 hash 的时候，hash值要跟 table长度-1 进行与操作, table长度为 2^n，也就是二进制100000， 2^n -1 也就是二进制 11111， 跟1与更不容易碰撞。
